...
- [ ] Add source/CSS coverage for the global focus ring, native Library row
  controls, view-mode pressed state, Ask AI status/names, and scalable tokens.
- [ ] Add Playwright keyboard coverage for Library list open/More separation and
  visible focus.
- [ ] Add Playwright geometry and 200% text-size coverage on both phone projects.
- [ ] Run the focused tests and record only the intended failures.

## Task 2: Implement native semantics and focus

- [ ] Add a theme-aware global `:focus-visible` treatment and remove the root-tab
  outline suppression.
- [ ] Split Library list rows into a primary native button and separate More
  button while preserving selection, Motion, shared-cover, and press feedback.
- [ ] Give Library view mode grouped pressed-state semantics.
- [ ] Make Ask AI configuration/submit controls native and named; announce busy,
  message, success, and error state changes appropriately.
- [ ] Add live status semantics to backup and model refresh feedback.

## Task 3: Harden target size and text reflow

- [ ] Raise audited frequent compact controls to a 44px interactive box without
  enlarging their glyphs.
- [ ] Add named `rem` typography tokens and migrate daily-path UI text.
- [ ] Add responsive wrapping/stacking for headers, Library metadata, Settings
  rows, and compact action groups under enlarged text.
- [ ] Inspect light/dark/sepia/custom-background, reduced-motion, software
...
- [ ] Run the Impeccable targeted scan on changed components and CSS.
