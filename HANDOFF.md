...
```text
https://frog-comp-metres-laura.trycloudflare.com
```

It is backed by:

```powershell
npm.cmd run start -- --hostname 127.0.0.1 --port 3033
C:\tmp\cloudflared.exe tunnel --protocol http2 --url http://127.0.0.1:3033
```
...
2. Restart `next start` on port `3033` or a new free port.
...
```powershell
$html=(Invoke-WebRequest -UseBasicParsing https://frog-comp-metres-laura.trycloudflare.com).Content
$css=$html | Select-String -Pattern '/_next/static/chunks/[^"'']+\.css' -AllMatches | ForEach-Object { $_.Matches.Value } | Select-Object -Unique
$css
foreach($u in $css){ $r=Invoke-WebRequest -UseBasicParsing "https://frog-comp-metres-laura.trycloudflare.com$u"; "$u $($r.StatusCode) $($r.Headers['Content-Type']) len=$($r.RawContentLength)" }
```
