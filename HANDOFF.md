...
- Base branch: `main`
- Latest code commit: `169d0dd` (`fix: clear nested epub canvas colors`)
- If branch HEAD is newer than `169d0dd`, that newer commit should be this handoff-only documentation update.
- Latest pushed branch state before this handoff update:
  ...
  - local branch includes `169d0dd`; push it before handing off if not already pushed
...
- Latest deployed Cloudflare Worker version:
  `372154f4-980f-4f44-a6e6-0a5874887ba5`.
...
After the latest code commit `169d0dd`, these passed:
...
- Cloudflare OpenNext deploy passed and published Worker version `372154f4-980f-4f44-a6e6-0a5874887ba5`.
...
最新代码提交以 169d0dd 为准：继 EPUB 深色模式文字修复后，又修复了部分书籍嵌套白色画布导致的白底白字；现在递归清除 iframe 内 div/main/section/article 的背景色，同时保留背景图片和图片元素。最新 Worker 版本是 372154f4-980f-4f44-a6e6-0a5874887ba5。下面较早提交与 Worker 版本仅为历史摘要。
