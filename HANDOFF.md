...
- Base branch: `main`
- Latest code commit: `2fc1299` (`fix: pin dark epub root canvas`)
- If branch HEAD is newer than `2fc1299`, that newer commit should be this handoff-only documentation update.
- Latest pushed branch state before this handoff update:
  ...
  - local branch includes `2fc1299`; push it before handing off if not already pushed
...
- Final deterministic strategy: dark/system-dark EPUB sessions pin the iframe
  `html/body` root canvas to the active dark theme background (currently
  `#171717`) in both epub.js theme CSS and inline styles. Nested layout
  containers remain transparent and text uses the paired light foreground.
- Light and sepia backgrounds still resolve to transparent, preserving the
  ambient reader appearance in those themes.
...
- Latest deployed Cloudflare Worker version:
  `d7c9023a-3fc1-44cf-8f95-f41b54ea2ed0`.
...
After the latest code commit `2fc1299`, these passed:
...
- Cloudflare OpenNext deploy passed and published Worker version `d7c9023a-3fc1-44cf-8f95-f41b54ea2ed0`.
...
最新代码提交以 2fc1299 为准：针对 iPhone Safari 即使透明后仍把 EPUB iframe 根画布画成白色的问题，深色/系统深色主题现在会把 html/body 根画布明确固定为当前深色背景，内部容器继续透明，文字使用配套浅色前景；浅色和护眼主题仍保持透明 ambient。最新 Worker 版本是 d7c9023a-3fc1-44cf-8f95-f41b54ea2ed0。下面较早提交与 Worker 版本仅为历史摘要。
